    ...
    const info = await page.evaluate(() => {
      ...
      const nodes = [
        ...footer.querySelectorAll(
          'button, .m-button, [role="button"], div, span'
        ),
      ]
        .map((el) => ({
          tag: el.tagName,
        ...
        .filter(
          (item) =>
            item.text ||
            String(item.cls || "").includes("submit") ||
            String(item.cls || "").includes("draft")
        )
        .slice(0, 30);
      return {
        hasFooter: !!document.querySelector(".video-batch-footer"),
        actionFooterBtnCount: document.querySelectorAll(
          ".video-batch-footer .action-footer-btn"
        ).length,
        submitCount: document.querySelectorAll(".video-batch-footer .submit")
          .length,
        draftCount: document.querySelectorAll(".video-batch-footer .draft")
          .length,
        nodes,
      };
    });
...
async function waitForToutiaoPublishFooterReady(page) {
  await pollPageUntil(
    ...
    () => {
      ...
      const footer = document.querySelector(".video-batch-footer") || document;
      return [
        ...footer.querySelectorAll('button, .m-button, [role="button"], div'),
      ].some((el) => {
        const text = norm(el.textContent);
        return (
          isReady(el) &&
          (text === "发布" || text === "存草稿" || text === "保存草稿")
        );
      });
    },
    WAIT_SELECTOR_APPEAR_MS,
...
async function clickToutiaoFooterAction(page, { draft }) {
  ...
  try {
    ...
    console.log(
      `[tt] ${label} selector 点击失败，尝试按文本兜底:`,
      e?.message || e
    );
    await dumpToutiaoPublishFooterState(
      page,
      `${draft ? "draft" : "publish"}-selector-fail`
    );
  }
...
async function setToutiaoCover(page, hasTagSelector, setPublishStage) {
  ...
  const isXiguaPosterUploaded = async () => {
    try {
      return await page.evaluate(() => {
        ...
        if (hasBg(document.querySelector(".xigua-poster-editor .bg")))
          return true;
        if (hasBg(document.querySelector(".base-content-wrap .bg")))
          return true;
        if (hasBg(document.querySelector(".base-content-wrap .preview")))
          return true;
        // 新版可能用 canvas 渲染裁剪结果
  ...
  console.log(
    "[tt] 准备点击封面弹窗确认 .Dialog-container .footer .m-button.red"
  );
  try {
    await clickReadyElement(
      page,
...
export default async function (page, data, window, event) {
  ...
  try {
    ...
    await page.keyboard.type(data.data.title || data.data.bt1 || "", {
      delay: 50,
    });
