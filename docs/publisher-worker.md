...
头条文章转存草稿在填写标题后直接把正文写入编辑器，不再等待始终未出现的“仅标题”初始保存请求；正文插图须先通过编辑器上传以取得平台地址，只有封面的图片则在正文写入和字数检查后通过单图封面入口上传。编辑器须能读到正文首尾及足够长度，平台字数不能为 0；完整正文保存请求也须包含首尾内容并返回业务码 0。首次完整正文保存可以没有 `pgc_id`，但必须再到草稿箱核对精确标题，不能仅凭本地输入或请求发出判定成功。图片工具栏按钮没有文字时，仅在标题与正文的公共编辑区内选择可见工具栏的第 12 个小图标按钮；无法唯一定位就报错，不点击页面其他按钮。图片面板的“本地上传”文件框可能没有 `accept` 属性，因此优先定位可见“本地上传”按钮或标签内的文件框；只有唯一一个明确接受图片的文件框时才回退选择，避免误用视频或普通附件上传框。只有封面的图片从 `.article-cover-images-wrap` 内的单图入口打开图片面板；等“已上传 1 张图片”且面板内“确定”可用后确认，再检查封面区出现新的 HTTPS 图片、正文图片未改变。正文插图也要先在同一图片面板内确认，再等待编辑器出现新图片。封面确认后等待新的草稿保存标记；若原标记一直可见，最终仍以重新打开精确标题草稿核验正文和封面为准。同账号、同内置浏览器的对照中，手动可见后台完整保存返回业务码 0，隐藏自动窗口的初始请求曾返回 7050。因此仅头条 Worker 文章“转存草稿”窗口保持可见，且不使用发布窗口的反检测脚本和 puppeteer-extra 注入；立即发布、其他平台和视频窗口策略不变。Worker 不自行拼接头条签名 URL，也不会在失败后自动重试。此流程仍须通过自动草稿的真实平台验收，尤其要回开草稿复核正文和封面。
