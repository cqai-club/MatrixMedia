...
头条文章转存草稿在填写标题后先聚焦仍为空的正文编辑器，使标题框失焦并触发初始自动保存；必须等“仅标题”保存返回业务码 0，页面显示“草稿已保存”，随后才写入正文。完整正文保存请求还必须携带草稿标识、返回业务码 0，最后到草稿箱核对标题。此顺序来自同账号、同内置浏览器的手动/自动请求对比：首次携带完整正文且没有草稿标识的请求曾返回 7050；只在标题框输入并保持焦点也未触发初始保存。Worker 不自行拼接头条签名 URL，也不会在失败后自动重试。该顺序修正尚需一次受控平台草稿验证，不应仅凭自动测试宣称已经修好。
